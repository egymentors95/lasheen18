...
class building_unit(models.Model):
    ...
    region_id= fields.Many2one('regions', string='Region',  store=True ) #related='building_id.region_id',
   
    
    project_id_new= fields.Many2one('project.worksite', string='Project' )
    account_analytic_id= fields.Many2one('account.analytic.account', 'Analytic Distribution')
    ...
    price_in_m2= fields.Float   ('Price in m²',)
    ...
    is_land = fields.Boolean(string="Is Land", default=False ,store=True)

    @api.onchange('price_in_m2', 'building_area_net')
    def _onchange_price_in_m2(self):
        for rec in self:
            if rec.price_in_m2 and rec.building_area_net:
                rec.pricing = rec.price_in_m2 * rec.building_area_net

    @api.onchange('ptype')               
    def _onchange_ahmady(self):           
        if self.ptype.name == 'Land':
            self.is_land = True
        else:
            self.is_land = False

    @api.onchange('project_id_new')
    def _onchange_ (self):
        self.region_id=self.building_id.region_id
    ...
    def make_reservation(self):
        for unit_obj in self:
            ...
            # building_area =  unit_obj.building_area
            account_analytic_id =unit_obj.account_analytic_id.id
            
            param_key = 'itsys_real_estate.income_account'
            param_value = self.env['ir.config_parameter'].sudo().get_param(param_key)
...
class FloorPlans(models.Model):
    ...
    def _compute_can_image_1024_be_zoomed(self):
        for record in self:
            ...
            # edit m_ahmady
    # def _get_video_embed_code(self, video_url):
    #         if "youtube.com" in video_url or "youtu.be" in video_url:
    #             youtube_id = re.search(r'(?:v=|/)([0-9A-Za-z_-]{11})', video_url)
    #             if youtube_id:
    #                 return f'<iframe width="560" height="315" src="https://www.youtube.com/embed/{youtube_id.group(1)}" frameborder="0" allow="accelerometer; autoplay; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>'
    #         elif "vimeo.com" in video_url:
    #             vimeo_id = re.search(r'vimeo.com/(\d+)', video_url)
    #             if vimeo_id:
    #                 return f'<iframe src="https://player.vimeo.com/video/{vimeo_id.group(1)}" width="640" height="360" frameborder="0" allow="autoplay; fullscreen" allowfullscreen></iframe>'
    #         return ""
    # @api.depends('video_url')
    # def _compute_embed_code(self):
    #     for image in self:
    #         image.embed_code = self._get_video_embed_code(image.video_url)
