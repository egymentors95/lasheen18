...
class building(models.Model):
    ...
    penthouse = fields.Integer('# Penthouse')
    ...
    project_id= fields.Many2one('project.worksite',string='Project',domain=[('parent_id','=',False)])
    ...
    def onchange_url(self):
        project_id = self.project_id.name.account_id.id
        if project_id :
            self.account_analytic_id=project_id
        else:
            self.account_analytic_id=project_id=""
    ...
    def action_create_units(self):
        ...
        if self.no_of_floors and self.props_per_floors:
            ...
            while i<=self.no_of_floors:
                print('i',i)
                print('self.no_of_floors',self.no_of_floors)
                j=1
                while j<=self.props_per_floors:
                    print('j',j)
                    print('self.props_per_floors',self.props_per_floors)
                    vals={
                    ...
                    j+=1
                i+=1
            for p in range(1,self.penthouse+1):
                vals={
                    'name':self.code+' - '+str('Last')+' - '+str(p),
                    'code': str('Last')+' - '+str(p),
                    'building_id':self.id,
                    'floor':'Last',
                    'is_property': True,
