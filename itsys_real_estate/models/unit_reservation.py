...
class unit_reservation(models.Model):
    ...
    image = fields.Binary(" Image", help="Upload your photo")
    ...
    def action_draft(self):
        self.message_post(
            body=_("Button 'Reset to Draft' was clicked for Draft %s" % self.id),
    ...
    def action_cancel(self):
        self.message_post(
            body=_("Button 'Cancel' was clicked for Cancel %s" % self.id),
            message_type='notification',
            subtype_xmlid='mail.mt_note',
        )
    ...
    def action_contract_ownership(self):
        self.message_post(
            body=_("Button 'Ownership Contract' was clicked for reservation %s" % self.id),
            message_type='notification',
            subtype_xmlid='mail.mt_note',
        )
        return {
            ...
            'context': {
                ...
                'default_reservation_id': self.id,
                'default_account_income': self.account_income.id,
    ...
    def action_contract_rental(self):
        self.message_post(
            body=_("Button 'Rental Contract' was clicked for Rental %s" % self.id),
            message_type='notification',
            subtype_xmlid='mail.mt_note',
        )
        param_key = 'itsys_real_estate.security_deposit_account'
        param_value = self.env['ir.config_parameter'].sudo().get_param(param_key) 
        print("mohamed elahmady ali", param_value) 
        account_security_deposit_id = int(param_value) if param_value and param_value.isdigit() else None 
        if account_security_deposit_id:
            account_exists = self.env['account.account'].sudo().search_count([('id', '=', account_security_deposit_id)])
            if not account_exists:
                raise UserError(_('The security deposit account with ID %s does not exist. test ahmady') % account_security_deposit_id)
        else:
            raise UserError(_('The security deposit account parameter is not configured or invalid. Please check your settings.'))
        ...
        return {
            ...
            'context': {
                ...
                'default_reservation_id': self.id,
                'default_account_income': self.account_income.id,
                'default_account_security_deposit': account_security_deposit_id,  # ahmady
